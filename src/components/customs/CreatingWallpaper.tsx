...
export default function CreatingWallpaper({ params }: { params: Promise<{ id: string }> }) {
            ...
            <ScrollArea>
              <div className="flex h-full min-h-28 flex-nowrap gap-6 overflow-hidden pb-4">
                <div className="relative">
                  ...
                  <div
                    onClick={() => setWallpaperType(undefined)}
                    className={clsx(
          ...
          <div className="mt-4 rounded-lg bg-slate-100 p-4">
            ...
            <div className="flex items-center gap-2">
              {[...Array(4)].map((_, idx) => (
                <div
                  key={idx}
                  onClick={() => setAmount(idx + 1)}
                  className={clsx(
                    "flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg bg-gray-200 text-center",
              ...
              {isLoading ? (
                ...
                <>
                  ...
                  <span className="ml-1 flex items-center gap-1 text-xs">
                    <Stars size={8} />
                    {`${session?.data?.user?.credits! > 1 ? session?.data?.user.credits + " credits" : session?.data?.user.credits + " credit"}`}
                  </span>
                </>
              )}
            </Button>
